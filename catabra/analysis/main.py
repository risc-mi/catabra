...
class Analyzer(CaTabRaBase):
    ...
    def __call__(
        ...
        if isinstance(self._invocation.table[0], Path):
            ...
            dataset_name = None
        target = self._invocation.target
        ...
        if isinstance(self._invocation.config_src, (str, Path)):
            ...
            config_value = io.load(self._invocation.config_src)
        else:
            config_value = {} # self._invocation.config
        ...
        if self._invocation.out is None:
            ...
            self._invocation.out = self._invocation.out.parent / (self._invocation.out.stem + '_catabra_' + self._invocation.start.strftime('%Y-%m-%d_%H-%M-%S'))
        else:
        ...
        with logging.LogMirror((self._invocation.out / CaTabRaPaths.ConsoleLogs).as_posix()):
            ...
            self._invocation.config_src = config_value
            if isinstance(self._invocation.config_src, dict):
                self._invocation.config_src = copy.deepcopy(self._invocation.config_src)
            else:
                if self._invocation.default_config not in DEFAULT_CONFIGS.keys():
                    raise ValueError('Default config must be one of "full", "basic" or "interpretable",'
                                      f' but found {self._invocation.default_config}.')
                self._invocation.config_src = cfg.add_defaults(self._invocation.config_src, default=DEFAULT_CONFIGS.get(self._invocation.default_config, {}))

            self._invocation.config_src = cfg.add_defaults(self._invocation.config_src)
            io.dump(self._invocation.config_src, self._invocation.out / CaTabRaPaths.Config)
            ...
            print('target', target)
            print('target', self._invocation.ignore)
            ...
            # copy training data
            copy_data = self._invocation.config_src.get('copy_analysis_data', False)
            ...
            # backend = None
            if y_train is not None and (self._invocation.config_src.get('time_limit') if self._invocation.time is None else self._invocation.time) != 0:
                automl = self._invocation.config_src.get('automl')
                if automl is not None:
                    backend = AutoMLBackend.get(automl, task=encoder.task_, config=self._invocation.config_src,
                    ...
                    explainer_name = self._invocation.config_src.get('explainer')
    ...
    def _make_training_plots(self, hist):
        static_plots = self._invocation.config_src.get('static_plots', True)
        interactive_plots = self._invocation.config_src.get('interactive_plots', False)
    ...
    def _make_ood_detector(self, x_train, y_train):
        ood_config = self._invocation.config_src.get('ood', None)
